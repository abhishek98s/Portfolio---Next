...
const Projects: React.FC<{ isHome: boolean }> = ({ isHome }) => {
    const [projecType, setProjecType] = useState<'all' | 'frontend' | 'fullstack'>('all');
    ...
    return (
        <section className={`${isHome && 'w-[100%] py-[80px] pb-[140px] overflow-x-hidden'} mx-auto`} id="projects">
            <div className={`${isHome && styles.project_wrpper} ${isHome && 'custom-container'} relative`}>
                ...
                {!isHome &&
                    <div className="text-[18px] font-semibold mb-[8px] color-primary-800">Others Projects</div>
                }
                ...
                <div className={`project-list gap-[24px] ${isHome && 'min-h-[200px]'} grid grid-cols-1 md:grid-cols-2 ${isHome && 'md:grid-cols-3'} relative z-20`}>
