    ...
    <section className='py-[80px]' id='skills'>
      <div className='custom-container'>
        ...
        <div className='grid grid-cols-3 md:grid-cols-5 lg:grid-cols-9 gap-y-[20px]'>
          {skillsArr.length &&
            skillsArr.map((skill: ISkill, index: number) => (
              <div
                key={index}
                className='skill px-[4px] pt-[16px] pb-[12px] border-primary-10 rounded-[4px] w-[108px]'
              >
                ...
                <div className='title text-center text-[16px] color-primary-400'>
                  {skill.title}
                </div>
              </div>
            ))}
